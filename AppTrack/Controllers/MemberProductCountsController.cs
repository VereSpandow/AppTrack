...
namespace AppTrack.Controllers
{
    ...
    public class MemberProductCountsController : BaseController
    {

        private ApplicationUserManager _userManager;
        private ApplicationRoleManager _roleManager;
        
        public MemberProductCountsController()
        {
        }
        public MemberProductCountsController(ApplicationUserManager userManager, ApplicationRoleManager roleManager)
        {
        ...
        public ActionResult Index(string submit = "submit")
        {
            ...
            MemberProductCountReportViewModel model = new MemberProductCountReportViewModel();
            using (var db = new DevProvidentIDOCEntities())
            {
                model.MemberProductCountReportListXTab = db.Database.SqlQuery<MemberProductCountReportXTab>("exec dbo.[LB_GetMemberProductCounts] @CountType, @StatusType, @isXTab",
            ...
            if (submit == "Download")
            {
                ...
                if (model.MemberProductCountReportListXTab.Count > 0)
                {
                    int i = 0;
                    string attachment = "attachment; filename=" + AdminID.ToString() + "-MemberRetentionByProductReport.csv";
                    HttpContext.Response.AddHeader("content-disposition", attachment);
                    var sw = new StreamWriter(new MemoryStream());
                    foreach (var line in model.MemberProductCountReportListXTab)
                    {
                        i = i + 1;
                        if (i == 1)
                        {
        ...
        [HttpGet]
        public JsonResult StackedBarChartProduct(string ct = "Type", string st = "All")
        {
            string countType = ct;
            ...
            using (var db = new DevProvidentIDOCEntities())
            {
                model.MemberProductCountReportList = db.Database.SqlQuery<MemberProductCountReport>("exec dbo.[LB_GetMemberProductCounts] @CountType, @StatusType, @isXTab",
                new SqlParameter("@CountType", countType),
                new SqlParameter("@StatusType", statusType),
            ...
            foreach (var listitem in model.MemberProductCountReportList)
            {
                i = i++;
                thisChartData.Add(new StackBarData(i, listitem.PeriodName, listitem.DepartmentName, listitem.MemberCount));
            }
